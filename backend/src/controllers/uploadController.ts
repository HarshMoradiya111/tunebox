  ...
  try {
    ...
    if (!targetPlaylistName) {
      ...
      if (fullText.includes("bollywood") || fullText.includes("hindi")) {
        ...
        targetPlaylistName = trackArtist || "My Uploads"; // Fallback to Artist Name
      }
    }
    ...
    if (!targetPlaylist) {
      ...
      if (!targetPlaylist.tracks.some(id => id.toString() === newSong._id.toString())) {
        targetPlaylist.tracks.push(newSong._id);
        targetPlaylist.totalTracks = targetPlaylist.tracks.length;
        if (!targetPlaylist.coverImage && albumArtUrl) {
          targetPlaylist.coverImage = albumArtUrl;
        }
      }
    }
