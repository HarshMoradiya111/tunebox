...
export const uploadTrack = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    // Auto-playlist step
    let autoPlaylist = await Playlist.findOne({ autoGenerated: true, name: trackArtist });
    if (!autoPlaylist) {
      autoPlaylist = new Playlist({
        spotifyId: `auto-playlist-${trackArtist.toLowerCase().replace(/[^a-z0-9]/g, '-')}-${Date.now()}`,
        name: trackArtist,
        description: `All tracks by ${trackArtist}`,
        coverImage: albumArtUrl,
        owner: "TuneBox",
        tracks: [newSong._id],
        totalTracks: 1,
        autoGenerated: true,
      ...
      autoPlaylist.tracks.push(newSong._id);
      autoPlaylist.totalTracks = autoPlaylist.tracks.length;
      if (!autoPlaylist.coverImage && albumArtUrl) {
        autoPlaylist.coverImage = albumArtUrl;
      }
    }
